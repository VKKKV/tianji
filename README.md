...
`history-compare` preset selection is intentionally exclusive: use exactly one of explicit `--left-run-id/--right-run-id`, `--latest-pair`, `--run-id ... --against-latest`, or `--run-id ... --against-previous`. Mixed preset combinations, negative per-side compare limits, and inverted compare score windows are rejected at parse time so the compare surface stays unambiguous.
