...
class HistoryListTests(unittest.TestCase):
    def test_history_list_contract_fixture_freezes_v1_vocabulary(self) -> None:
        list_fixture = cast(
        ...
        self.assertEqual(set(payload[0]), set(list_fixture))
        self.assertEqual(payload[0]["schema_version"], list_fixture["schema_version"])
        self.assertEqual(payload[0]["mode"], list_fixture["mode"])
        self.assertEqual(set(api_fixture), {"api_version", "data", "error"})
        self.assertEqual(
            set(cast(dict[str, object], api_fixture["data"])),
            {"resource", "items", "item_contract_fixture"},
        )
        self.assertEqual(
