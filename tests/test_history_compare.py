...
class HistoryCompareTests(unittest.TestCase):
    def test_history_compare_contract_fixture_freezes_v1_vocabulary(self) -> None:
        compare_fixture = cast(
            dict[str, object], load_contract_fixture("history_compare_v1.json")
        )
        api_compare_fixture = cast(
            dict[str, object], load_contract_fixture("local_api_compare_v1.json")
        )
    ...
    """
        ...
        left_payload = cast(dict[str, object], payload["left"])
        right_payload = cast(dict[str, object], payload["right"])
        diff_payload = cast(dict[str, object], payload["diff"])
        left_fixture = cast(dict[str, object], compare_fixture["left"])
        right_fixture = cast(dict[str, object], compare_fixture["right"])
        diff_fixture = cast(dict[str, object], compare_fixture["diff"])
        left_top_scored_event = cast(
            dict[str, object], left_payload["top_scored_event"]
        )
        left_top_scored_event_fixture = cast(
            dict[str, object], left_fixture["top_scored_event"]
        )
        left_top_intervention = cast(
            dict[str, object], left_payload["top_intervention"]
        )
        left_top_intervention_fixture = cast(
            dict[str, object], left_fixture["top_intervention"]
        )
        left_top_event_group = cast(dict[str, object], left_payload["top_event_group"])
        left_top_event_group_fixture = cast(
            dict[str, object], left_fixture["top_event_group"]
        )
        left_evidence_chain = cast(
            list[dict[str, object]], left_top_event_group["evidence_chain"]
        )
        left_evidence_chain_fixture = cast(
            list[dict[str, object]], left_top_event_group_fixture["evidence_chain"]
        )
        top_event_group_evidence_diff = cast(
            dict[str, object], diff_payload["top_event_group_evidence_diff"]
        )
        top_event_group_evidence_diff_fixture = cast(
            dict[str, object], diff_fixture["top_event_group_evidence_diff"]
        )

        self.assertEqual(set(payload), set(compare_fixture))
        self.assertEqual(set(left_payload), set(left_fixture))
        self.assertEqual(set(right_payload), set(right_fixture))
        self.assertEqual(set(diff_payload), set(diff_fixture))
        self.assertEqual(set(left_top_scored_event), set(left_top_scored_event_fixture))
        self.assertEqual(set(left_top_intervention), set(left_top_intervention_fixture))
        self.assertEqual(set(left_top_event_group), set(left_top_event_group_fixture))
        self.assertEqual(
            set(left_evidence_chain[0]), set(left_evidence_chain_fixture[0])
        )
        self.assertEqual(
            set(top_event_group_evidence_diff),
            set(top_event_group_evidence_diff_fixture),
        )
        self.assertEqual(set(api_compare_fixture), {"api_version", "data", "error"})
        self.assertEqual(
            set(cast(dict[str, object], api_compare_fixture["data"])),
            {"resource", "status", "v1_routes", "payload_fixture"},
        )
        self.assertEqual(
            cast(dict[str, object], api_compare_fixture["data"])["resource"],
            "/api/v1/compare?left_run_id=<id>&right_run_id=<id>",
        )
        self.assertEqual(
            cast(dict[str, object], api_compare_fixture["data"])["v1_routes"],
