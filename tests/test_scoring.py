...
class ScoringTests(unittest.TestCase):
    def test_score_event_exposes_explicit_im_fa_semantics(self) -> None:
        ...
        self.assertEqual(scored.impact_score, 13.8)
        self.assertEqual(scored.field_attraction, 7.66)
        self.assertEqual(scored.divergence_score, 19.31)
        self.assertIn("Im=13.8", scored.rationale)
        self.assertIn("Fa=7.66", scored.rationale)
        self.assertIn("im_field_impact_scaling=0.24", scored.rationale)
        ...
        self.assertNotIn("im_title_salience=", " ".join(scored.rationale))
    ...
    def test_score_event_rewards_actor_and_region_title_salience(self) -> None:
        baseline_event = NormalizedEvent(
            event_id="evt-title-salience-baseline",
            source="fixture:test",
            title="Neutral update",
            summary="Neutral summary language.",
            link="https://example.com/title-salience-baseline",
            published_at="2026-03-22T12:12:30Z",
            keywords=["neutral", "update", "brief"],
            actors=["usa"],
            regions=["united-states"],
            field_scores={
        ...
        title_salient_event = NormalizedEvent(
            event_id="evt-title-salience-boosted",
            source="fixture:test",
            title="United States update",
            summary="Neutral summary language.",
            link="https://example.com/title-salience-boosted",
            published_at="2026-03-22T12:12:31Z",
            keywords=["neutral", "update", "brief"],
            actors=["usa"],
            regions=["united-states"],
            field_scores={
        ...
        baseline_scored = score_event(baseline_event)
        title_salient_scored = score_event(title_salient_event)

        self.assertEqual(
            baseline_scored.field_attraction, title_salient_scored.field_attraction
        )
        self.assertAlmostEqual(
            title_salient_scored.impact_score - baseline_scored.impact_score,
            0.5,
            places=2,
        )
        self.assertNotIn("im_title_salience=", " ".join(baseline_scored.rationale))
        self.assertIn("im_title_salience=0.5", title_salient_scored.rationale)

    def test_score_event_applies_field_specific_impact_scaling(self) -> None:
        economy_event = NormalizedEvent(
            event_id="evt-field-scaling-economy",
            source="fixture:test",
            title="Neutral update",
            summary="Neutral summary language.",
            link="https://example.com/field-scaling-economy",
            published_at="2026-03-22T12:12:32Z",
            keywords=["neutral", "update", "brief"],
            actors=["observer"],
            regions=["unknown-region"],
            field_scores={
        ...
        conflict_event = NormalizedEvent(
            event_id="evt-field-scaling-conflict",
            source="fixture:test",
            title="Neutral update",
            summary="Neutral summary language.",
            link="https://example.com/field-scaling-conflict",
            published_at="2026-03-22T12:12:33Z",
            keywords=["neutral", "update", "brief"],
            actors=["observer"],
            regions=["unknown-region"],
            field_scores={
        ...
        economy_scored = score_event(economy_event)
        conflict_scored = score_event(conflict_event)

        self.assertEqual(
            economy_scored.field_attraction, conflict_scored.field_attraction
        )
        self.assertAlmostEqual(
            conflict_scored.impact_score - economy_scored.impact_score,
            0.21,
            places=2,
        )
        self.assertIn("im_field_impact_scaling=0.1", economy_scored.rationale)
        self.assertIn("im_field_impact_scaling=0.31", conflict_scored.rationale)
