...
persisted operator ergonomics, the shipped local API read surface, or the first
TUI slice as hypothetical frontier work. The selected near-term slice is Phase 5
parity and verification work on the already-shipped read-only TUI, while Phase 4
remaining work stays limited to small wording polish unless a new concrete
operator gap appears, and any new scoring branch should still wait for a fresh
mixed-field weakness beyond the already-shipped near-tie and diffuse-third-field
coverage.
...
- a first contract draft exists in `contracts/tui-contract.md`
...
- keep the already-shipped shared read-only detail/compare lens controls aligned
  ...
  behavior and keeping this phase focused on verification-first hardening around:
  - `dominant_field`
  - `limit_scored_events`
  - `group_dominant_field`
  - `limit_event_groups`
  - `only_matching_interventions`
- keep the history list pane on persisted truth, now with the richer persisted
  triage fields already visible in list rows, rather than introducing in-TUI
  list filtering
- strengthen automated TUI verification, especially storage-backed coverage for
  compare flow, projected-empty states, persisted navigation, and lens-driven
  read-only behavior
...
3. continue the read-only Rich TUI carefully by keeping the history list on persisted truth, preserving the now-shipped richer persisted triage rows, and hardening storage-backed verification around current history/detail/compare semantics rather than inventing a second command model
...
Shipped contract notes now live in `contracts/local-api-contract.md`, `contracts/daemon-contract.md`, `contracts/tui-contract.md`, and `contracts/web-ui-contract.md`; the local API is now implemented as a read-first loopback surface hosted by the daemon, while Phase 5 already has an early Rich-based read-only implementation and the web UI remains optional.
...
- update `scoring-spec.md` before deep code edits
...
- `scoring-spec.md` now defines bounded actor/region title salience and bounded
...
2. write a short factor inventory note inside `scoring-spec.md`
...
- the formula remains easy to explain in one short section of `scoring-spec.md`
...
- **Local API work** = keep the shipped read-first loopback surface narrow and
  stable inside the existing daemon boundary; `contracts/local-api-contract.md` now
  documents implemented routes instead of standing in for missing near-term
  foundation work
- **TUI next work** should keep refining that shipped read-only slice and reuse
  the current storage/artifact vocabulary in `contracts/tui-contract.md`
